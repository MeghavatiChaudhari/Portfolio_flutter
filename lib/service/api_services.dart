...
class ApiServices {
  ...
  Future<Map<String, dynamic>?> getHistoricData(
      String cityname, String fromDate, String toDate) async {
    try {
      print(
          "Trying to get Historical :http://api.weatherapi.com/v1/history.json?key=d771530ea2c04342a4a114808240807&days=7&q=$cityname&dt=$fromDate&end_dt=$toDate");

      var res = await http.get(Uri.parse(
          "http://api.weatherapi.com/v1/history.json?key=d771530ea2c04342a4a114808240807&days=7&q=$cityname&dt=$fromDate&end_dt=$toDate"));
      if (res.statusCode == 200) {
        return json.decode(res.body);
      } else {
        print("i  am in else");
